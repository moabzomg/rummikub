...
export function sortHand(hand, mode = SORT_MODES.COLOR) {
  if (mode === SORT_MODES.MANUAL) return [...hand];

  return [...hand].sort((a, b) => {
    // Always push jokers to the end
    if (a.isJoker) return 1;
    if (b.isJoker) return -1;

    if (mode === SORT_MODES.NUMBER) {
    ...
    // Default: COLOR sort (your current behavior)
