...
export function drawTile(state, playerIdx) {
  if (state.pool.length === 0) {
    return advanceTurn({
      ...state,
      log: [...state.log, "Pool empty — skipping turn."],
    });
  }
  ...
  return advanceTurn({
    ...
    log: [
      ...state.log,
      `${name} drew a tile. (${newPool.length} left in pool)`,
    ],
  });
}
...
function calcScores(state) {
  ...
  return state.hands.map((h, i) => {
    ...
    return i === state.winner
      ? state.hands.reduce((s, hh, j) => (j !== i ? s + handValue(hh) : s), 0)
      : -val;
  });
}
...
export function commitPlay(state, newBoard, newHand, playerIdx) {
  if (!isValidBoard(newBoard)) {
    return {
      ...state,
      error: "Invalid board — sets must be valid groups or runs!",
    };
  }
  ...
  const placedFromHand = [...newBoardIds].filter(
    (id) => !prevBoardIds.has(id) && prevHandIds.has(id),
  );
  // Tiles removed from board to hand (not allowed!)
  const takenFromBoard = [...prevBoardIds].filter(
    (id) => !newBoardIds.has(id) && newHandIds.has(id),
  );

  if (takenFromBoard.length > 0) {
    return {
      ...state,
      error: "Cannot take tiles from the board into your hand!",
    };
  }
  ...
  if (!state.hasInitialMeld[playerIdx]) {
    const placedTiles = prevHand.filter(
      (t) => !newHand.find((h) => h.id === t.id),
    );
    const placedValue = handValue(placedTiles);
    if (placedValue < 30) {
      return {
        ...state,
        error: `Initial meld must score at least 30 points! (You placed ${placedValue})`,
      };
    }
    ...
    if (usedBoardTile) {
      return {
        ...state,
        error: "Initial meld must use tiles from your hand only!",
      };
    }
  }

  const newHasMeld = state.hasInitialMeld.map((v, i) =>
    i === playerIdx ? true : v,
  );
  const newHands = state.hands.map((h, i) => (i === playerIdx ? newHand : h));
  ...
  let newState = {
    ...
    log: [
      ...state.log,
      `${name} placed ${tilesPlaced} tile${tilesPlaced !== 1 ? "s" : ""}.`,
    ],
  };
