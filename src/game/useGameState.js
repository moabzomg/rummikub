...
import {
  createDeck, isValidBoard, isValidSet, initialMeldValue,
  handValue, computeAIMove, tileValue
} from '../game/logic.js';
...
export function useGameState() {
  ...
  const playTiles = useCallback(() => {
    ...
    if (!player.hasInitialMeld) {
      ...
      if (val < 30) {
        setMessage(`❌ Initial meld must total at least 30 points (yours: ${val})`);
        return;
      }
    }
  ...
  const doAITurn = useCallback((playerIdx) => {
    setPlayers(prev => {
      const updated = [...prev];
      ...
      if (move.type === 'draw') {
          ...
          for (const addition of move.additions) {
            if (!playedIds.has(addition.tile.id)) {
              if (addition.position === 'end') {
                newSets[addition.setIndex] = [...newSets[addition.setIndex], addition.tile];
              } else {
                newSets[addition.setIndex] = [addition.tile, ...newSets[addition.setIndex]];
              }
        ...
        setBoard(newSets);
        addLog(`AI played ${playedIds.size} tile(s). (${player.hand.length} left)`);
  ...
  return {
    ...
    board, setBoard,
    pool,
    selected, toggleSelectTile, setSelected,
    stagingBoard, setStagingBoard,
    message, setMessage,
    log,
    winner,
    aiThinking,
