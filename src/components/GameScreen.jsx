    ...
    <div style={styles.root}>
        ...
        <div style={styles.selPreview}>
          {selectedTiles.length === 0 && (
            <span style={{ color: "var(--text-muted)", fontSize: "12px" }}>
              {cp.isAI
                ? "AI is playing..."
                : "Click tiles from your hand to select them"}
            </span>
          )}
          {selectedTiles.map((t) => (
          ...
          {selectedTiles.length >= 3 && (
            <span style={setValid ? styles.validLabel : styles.invalidLabel}>
              {setValid
                ? `✓ Valid set${!cp.hasInitialMeld ? ` · ${meldVal}pts` : ""}`
                : "✗ Invalid"}
            </span>
          )}
        ...
        {!cp.isAI && (
          <>
            ...
            <button
              style={btn({
                background:
                  setValid && (cp.hasInitialMeld || meldVal >= 30)
                    ? "#4ade80"
                    : "var(--bg-surface)",
                color:
                  setValid && (cp.hasInitialMeld || meldVal >= 30)
      ...
      {aiThinking && (
        <div style={styles.aiOverlay}>
          <div style={styles.aiBox}>🤖 AI is thinking...</div>
        </div>
      )}
    </div>
  );
