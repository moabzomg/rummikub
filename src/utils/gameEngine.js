...
export function tileVal(t) {
  return t.isJoker ? 30 : t.num;
}
export function handVal(h) {
  return h.reduce((s, t) => s + tileVal(t), 0);
}
...
export function isValid(tiles) {
  return tiles.length >= 3 && (isRun(tiles) || isGroup(tiles));
}
...
export function sortSet(tiles) {
  ...
  const colorSlots = [...norm].sort(
    (a, b) => (COLOR_ORDER[a.color] || 0) - (COLOR_ORDER[b.color] || 0),
  );
...
export function sortHand(hand, mode) {
  ...
  return [...hand].sort((a, b) => {
    ...
    return a.num !== b.num
      ? a.num - b.num
      : (COLOR_ORDER[a.color] || 0) - (COLOR_ORDER[b.color] || 0);
  });
}
  ...
  for (let si = 0; si < board.length; si++) {
    ...
    for (const ji of jokerIdxs) {
      ...
      for (const ht of hand) {
        ...
        if (
          ht.num === jokerNum &&
          (jokerColor === null || ht.color === jokerColor)
        ) {
          const newSet = [...set];
          newSet[ji] = ht;
          if (isValid(newSet)) {
...
export function findExtensions(hand, board) {
  ...
  for (let si = 0; si < board.length; si++) {
    ...
    for (const t of hand) {
      if (isValid([t, ...set]))
        results.push({ si, pos: "start", tile: t, val: tileVal(t) });
      if (isValid([...set, t]))
        results.push({ si, pos: "end", tile: t, val: tileVal(t) });
  ...
  while (improved) {
    ...
    if (exts.length > 0) {
      ...
      if (ext.pos === "start") b[ext.si] = sortSet([ext.tile, ...b[ext.si]]);
        ...
        b[ext.si] = sortSet([
          ...b[ext.si].slice(0, ext.insertAt),
          ext.tile,
          ...b[ext.si].slice(ext.insertAt),
        ]);
      improved = true;
      moved = true;
    }
...
export function applyHint(hint, pendingHand, pendingBoard) {
  ...
  if (hint.exts && hint.exts.length > 0) {
    ...
    if (ext.pos === "start")
      board[ext.si] = sortSet([ext.tile, ...board[ext.si]]);
    else if (ext.pos === "end")
      board[ext.si] = sortSet([...board[ext.si], ext.tile]);
    else
      board[ext.si] = sortSet([
        ...board[ext.si].slice(0, ext.insertAt),
        ext.tile,
        ...board[ext.si].slice(ext.insertAt),
      ]);
  }
  if (hint.jrep) {
